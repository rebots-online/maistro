...
class ModelTrainer:
    ...
    def train_model(self, data_yaml_path, epochs=100, imgsz=640):
        """Train YOLOv12 model with attention-based architecture"""
        logger.info("Starting model training with YOLOv12")
        ...
        model = YOLO('yolo12n.pt')  # Load pretrained YOLOv12 model

        # Train the model with YOLOv12-specific optimizations
        try:
            results = model.train(
                data=data_yaml_path,
                epochs=epochs,
    ...
    def validate_model(self, data_yaml_path):
        ...
        try:
            model = YOLO('runs/detect/sheet_music_detector_v12/weights/best.pt')
